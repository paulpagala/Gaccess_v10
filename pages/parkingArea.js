...
export default function ParkingArea() {
  ...
  const [bookedArray, setBookedArray] = React.useState([]);
  const sortedArray = bookedArray.sort((a, b) => {
    // const timeA = new Date("1970-01-01T" + a.time).getTime();
    // const timeB = new Date("1970-01-01T" + b.time).getTime();
    const timeA = new Date("January 1, 1970 " + a.time);
    const timeB = new Date("January 1, 1970 " + b.time);
    return timeA - timeB;
  });
  const emails = sortedArray.map(({ userEmail }) => userEmail);
  ...
  function refresh() {
    ...
    FindSlotNumbers();
    // setResultSSO(groupBySize(emails, lengthsOfParkingFloor))
  ...
  function FindSlotNumbers() {
    ...
    fetch(
      "https://zh66xn42vk.execute-api.ap-southeast-1.amazonaws.com/stage/slots",
      options
    )
      .then((response) => response.json())
      .then((response) => {
        ...
        {weekOnCalendar ? setBookedArray(
          response.bookedSlots
            .filter((area) => area.calendarRestriction === day+weekOnCalendar)
            .filter((area) => area.parkingArea === props.index)
        ) : setBookedArray(response.bookedSlots
          .filter((area) => area.calendarRestriction === presentDay+presentWeek)
          .filter((area) => area.parkingArea === props.index))}
      })
      .catch(() => {
        // setError("Failed to retrieve from api");
  ...
  // const [resultSSO,setResultSSO]= React.useState(groupBySize(emails, lengthsOfParkingFloor))
  const resultSSO = groupBySize(emails, lengthsOfParkingFloor)
              ...
              <StyledTableRow key={row}>
                ...
                <StyledTableCell align="left">
                  <Box sx={{ width: 200 }}>
                    ...
                    {result[indexParkingFloor][index] === "Booked" ?
                    <Typography component="subtitle1" variant="subtitle1" sx={{ ml: 0.5 }} >
                      ...
                      {/* {result[indexParkingFloor][index] === "Booked" ? {result[indexParkingFloor][index]} by {resultSSO[indexParkingFloor][index]} */}
                      {result[indexParkingFloor][index]} by {resultSSO[indexParkingFloor][index]}
                    </Typography>
                    :
                    <Typography component="subtitle1" variant="subtitle1" sx={{ ml: 0.5 }} >
                      {/* {slotStatus[indexParkingFloor + index + indexParkingFloor]} */}
